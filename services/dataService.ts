...
// Delete helper for local storage
const deleteLocalData = <T extends { id: string }>(key: string, id: string): boolean => {
  const current = getLocalData<T>(key, []);
...
export const executeTransaction = async <T>(
  ...
  try {
    ...
    console.error("[Transaction] Failed at operation", completedOps.length, error);
    if (rollback) {
      try {
        await rollback();
...
// DELETE function for BOM Nodes
export const deleteBomNode = async (bomId: string, wbs: string): Promise<boolean> => {
  const allBoms = getLocalData<BillOfMaterials>(LS_KEYS.BOMS, MOCK_BOMS);

  const bomIndex = allBoms.findIndex(b => b.id === bomId);
  if (bomIndex === -1) return false;

  const bom = allBoms[bomIndex];
  const itemIndex = bom.items.findIndex(i => i.wbs === wbs);
  if (itemIndex === -1) return false;

  // Remove the item
  bom.items.splice(itemIndex, 1);

  // Also remove any children (items with WBS starting with this prefix)
  bom.items = bom.items.filter(i => !i.wbs.startsWith(`${wbs}.`));
